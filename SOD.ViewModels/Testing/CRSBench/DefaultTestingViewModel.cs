...
using SensorViewModel = SOD.ViewModels.Testing.CRSBench.Sensors.SensorViewModel;

namespace SOD.ViewModels.Testing.CRSBench
{
    public class DefaultTestingViewModel : ReactiveObject, IActivatableViewModel
    {
        private App.Benches.CRSBench.Bench _bench;
        ...
        {
            _bench = (App.Benches.CRSBench.Bench)testBenchService.GetTestBench();
            ...
            UpdateChart();
            UpdateSensors();
            InfoMessage = localizationService["Testing.CRSBench.Step1"];

            bus.Subscribe<App.Benches.CRSBench.Messages.SelectedTestMessage>(m =>
            {
                if (m.TestType != App.Testing.TestType.Functional)
                {
            ...
            StartTest = ReactiveCommand.Create(() =>
            {
                ...
                {
                    InfoMessage = localizationService["Testing.CRSBench.Step3"];
                    exposureCounter = 0;
                    ExposureTime = "00:00:00";
            ...
            Exposure = ReactiveCommand.Create(() =>
            {
                ...
                {
                    InfoMessage = localizationService["Testing.CRSBench.Step4"];
                    exposureCounter++;
                    IsExposure = true;
                ...
                {
                    InfoMessage = localizationService["Testing.CRSBench.Step5"];
                    IsExposure = false;
            ...
            Result = ReactiveCommand.CreateFromTask(async () =>
            {
                ...
                if (_bench.CurrentTest?.TestType==App.Testing.TestType.Strength)
                {
                    vm = new Dialogs.StrengthTestResultViewModel(sensorService, localizationService, dialogService, Observable.Return(isAddTestToReport),
                                                                    _bench, ((App.Testing.Strength.Test)_bench.CurrentTest).TestResult.PostResults.FirstOrDefault());
                }
                else if (_bench.CurrentTest?.TestType == App.Testing.TestType.Leakage)
                {
                    vm = new Dialogs.LeakageTestResultViewModel(sensorService, dialogService, Observable.Return(isAddTestToReport),
                                                                    _bench, ((App.Testing.Leakage.Test)_bench.CurrentTest).TestResult);
                }
                var result = await dialogService.ShowDialogAsync("CRSBenchTestResult", vm);
                if (result!=null)
                {
                    //_bench.CalculateTestResult();
